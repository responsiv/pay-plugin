...
class PaypalPro extends GatewayBase
{
    ...
    {
        $host->test_mode = true;
        if ($paidStatus = InvoiceStatus::getStatusPaid())
            $host->invoice_status_id = $paidStatus->id;
    }
    ...
    public function processPaymentForm($data, $host, $invoice, $isAdmin = false)
    {
        ...
        try {
            ...
            $fields['CVV2'] = array_get($data, 'CSC');
            $fields['AMT'] = $invoice->total;
            $fields['ISSUENUMBER'] = array_get($data, 'issue_number');
            $fields['CURRENCYCODE'] = Settings::get('currency_code', 'USD');
